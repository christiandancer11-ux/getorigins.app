...
import { Award, QrCode, Plus, MessageCircle, Search, Loader2, CheckCircle2, ExternalLink, ArrowRight } from 'lucide-react';
...
export default function PsaCertScan() {
  ...
  // Find card in Origins by cert_number
  const { data: card, isLoading } = useQuery({
    queryKey: ['card-by-cert', cert],
    queryFn: async () => {
  ...
  // If logged in, fetch user's cards that don't have a cert yet (for linking)
        ...
        <div className="relative pt-20 pb-12 px-6">
          ...
          <div className="relative max-w-2xl mx-auto text-center">
            ...
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-amber-400/10 border border-amber-400/20 text-amber-400 text-xs font-semibold mb-5">
              <Award className="w-3.5 h-3.5" />PSA Cert #{cert}
            </div>

            {card.image_url && (
              <div className="w-32 h-44 mx-auto rounded-xl overflow-hidden border-2 border-amber-400/20 shadow-xl shadow-amber-400/10 mb-6">
            ...
            {card.grading_company && card.grade && (
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-lg bg-amber-400/10 border border-amber-400/20 mb-5">
                <Award className="w-3.5 h-3.5 text-amber-400" />
                <span className="text-sm font-semibold text-amber-400">{card.grading_company} {card.grade}</span>
              </div>
            )}
  ...
  // ── NO CARD FOUND — show register or link options ───────────────────────
  return (
      ...
      <div className="max-w-lg mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-muted-foreground">
            This PSA slab hasn't been registered on Origins. Be the first owner to add it and start its story.
          </p>
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {!currentUser && (
            <motion.div key="login" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
              <Button onClick={() => base44.auth.redirectToLogin(window.location.href)} className="w-full h-12 bg-primary text-primary-foreground hover:bg-primary/90 gap-2">
                Sign in to Register This Card
                <ArrowRight className="w-4 h-4" />
              </Button>
              <a href={`https://www.psacard.com/cert/${cert}`} target="_blank" rel="noopener noreferrer">
                <Button variant="outline" className="w-full border-border/50 gap-2">
                  <ExternalLink className="w-4 h-4" />View on PSA Registry
                </Button>
              </a>
            </motion.div>
          )}

          {/* Logged in — link or register */}
          {currentUser && !linkSuccess && (
            <motion.div key="options" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">

              {/* Option 1: Register brand new */}
              <div className="rounded-2xl border border-primary/20 bg-primary/5 p-5 space-y-3">
                <h3 className="font-semibold text-foreground flex items-center gap-2">
                  <QrCode className="w-4 h-4 text-primary" />Register as New Card
                </h3>
                <p className="text-sm text-muted-foreground">Start fresh — AI will identify the card from your photos and link it to this PSA cert automatically.</p>
                <Button
                  onClick={() => navigate(`/register?cert=${cert}&grader=PSA`)}
              ...
              {/* Option 2: Link to existing card */}
              {myCards.length > 0 && (
                <div className="rounded-2xl border border-border/50 bg-card p-5 space-y-3">
                  <h3 className="font-semibold text-foreground flex items-center gap-2">
                    <Award className="w-4 h-4 text-amber-400" />Link to an Existing Card
                  </h3>
                  <p className="text-sm text-muted-foreground">Already have this card in Origins? Link this PSA cert to it.</p>
                  <div className="space-y-2 max-h-48 overflow-y-auto">
                    {myCards.map(c => (
                      <button
                        key={c.id}
                        onClick={() => setSelectedCardId(c.id)}
                        className={`w-full flex items-center gap-3 p-3 rounded-xl border transition-colors text-left ${selectedCardId === c.id ? 'border-primary/50 bg-primary/10' : 'border-border/50 hover:border-primary/30 bg-secondary/30'}`}
                      >
                        ...
                        {selectedCardId === c.id && <CheckCircle2 className="w-4 h-4 text-primary ml-auto shrink-0" />}
                      </button>
                    ))}
                  </div>
                  <Button
                    ...
                    className="w-full bg-amber-400 text-black hover:bg-amber-400/90 gap-2"
                  >
                    {linking ? <Loader2 className="w-4 h-4 animate-spin" /> : <Award className="w-4 h-4" />}
                    {linking ? 'Linking...' : 'Link PSA Cert to This Card'}
                  </Button>
                </div>
              )}
          ...
          {linkSuccess && (
            <motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-8">
              ...
              <p className="font-semibold text-foreground">PSA cert linked!</p>
