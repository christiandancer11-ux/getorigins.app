...
export default function RegisterCard() {
  ...
  const createMutation = useMutation({
    mutationFn: async (data) => {
      ...
      const { data: card } = await createCard(payload);
      return card;
    },
  ...
  const uploadFile = async (file, setSide) => {
    ...
    try {
      ...
      const file_url = `https://zdwlmuhqlbtltqgbzrwa.supabase.co/storage/v1/object/public/card-images/${path}`;
  ...
  const handleAnalyze = async (front, back, condition) => {
    ...
    // TODO: AI analysis - temporarily using manual fallback
    setAiResult({
  ...
  const saveCard = async (result, overrideFront, overrideBack) => {
    ...
    const mapped = {
      ...
      image_back_url: cardData.image_back_url || '',
      // Add other fields as needed
    };
    createMutation.mutate(mapped);
  };
