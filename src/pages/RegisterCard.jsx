...
export default function RegisterCard() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    createMutation.mutate({ ...form, unique_code: generateCode() });
  };
  ...
  return (
    <div className="min-h-screen pt-24 pb-12 px-6">
      <div className="max-w-xl mx-auto">
        ...
        <p className="text-muted-foreground mb-8">Upload a photo and let AI identify your card automatically.</p>
              ...
              {form.image_url ? (
                <div className="relative w-full aspect-[3/4] max-w-[200px] rounded-xl overflow-hidden border border-border">
                  ...
                  <button type="button" onClick={() => update('image_url', '')}
                    className="absolute top-2 right-2 w-6 h-6 rounded-full bg-background/80 flex items-center justify-center text-xs text-foreground">✕</button>
                </div>
              ) : (
                <label className="flex flex-col items-center justify-center w-full aspect-[3/4] max-w-[200px] rounded-xl border-2 border-dashed border-border hover:border-primary/30 cursor-pointer transition-colors bg-muted/20">
                  ...
                  {uploading ? <Loader2 className="w-6 h-6 text-muted-foreground animate-spin" /> : (
                    <><Upload className="w-6 h-6 text-muted-foreground mb-2" /><span className="text-xs text-muted-foreground">Upload photo</span></>
                  )}
                </label>
              )}
            </div>
            {form.image_url && (
              <AiAutoFillButton imageUrl={form.image_url} onFill={handleAiFill} />
            )}
          </div>
